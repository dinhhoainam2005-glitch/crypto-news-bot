...
def check_fred():
    ...
    try:
        ...
        fred_online = False
    print(f"FRED: {'Online' if fred_online else 'Offline'}")
    return fred_online
...
def check_gdelt():
    ...
    try:
        ...
        gdelt_online = False
    print(f"GDELT: {'Online' if gdelt_online else 'Offline'}")
    return gdelt_online
...
KINH_TE_EVENTS = [
    {
        'id': 'nfp_2026-06-05', 'name': '💼 Non-Farm Payrolls (T5)',
        'date': '2026-06-05', 'time': '19:30', 'impact': '🔴 HIGH',
        'desc': 'Bao cao viec lam phi nong nghiep My.',
        'fred_series': 'UNRATE',
        'result_text': 'Ty le that nghiep thuc te: {value}% (ky truoc: {prev}%)'
    },
    {
        'id': 'cpi_2026-06-11', 'name': '📊 CPI Report (T5)',
        'date': '2026-06-11', 'time': '19:30', 'impact': '🔴 HIGH',
        'desc': 'Chi so gia tieu dung - thước do lam phat.',
        'fred_series': 'CPIAUCSL',
        'result_text': 'CPI thuc te: {value} (ky truoc: {prev})'
    },
    {
        'id': 'ppi_2026-06-12', 'name': '🏭 PPI Report (T5)',
        'date': '2026-06-12', 'time': '19:30', 'impact': '🟡 MEDIUM',
        'desc': 'Chi so gia san xuat.',
        'fred_series': 'PPIACO',
        'result_text': 'PPI thuc te: {value} (ky truoc: {prev})'
    },
    {
        'id': 'fomc_2026-06-18', 'name': '🏦 FOMC Rate Decision (T6)',
        'date': '2026-06-18', 'time': '01:00', 'impact': '🔴 HIGH',
        'desc': 'Quyet dinh lai suat Fed - SU KIEN QUAN TRONG NHAT.',
        'fred_series': 'DFF',
        'result_text': 'Lai suat Fed thuc te: {value}% (ky truoc: {prev}%) - Fed {action}'
    },
    {
        'id': 'gdp_2026-06-25', 'name': '📊 GDP Q2 2026 (Final)',
        'date': '2026-06-25', 'time': '19:30', 'impact': '🔴 HIGH',
        'desc': 'Tang truong kinh te My.',
        'fred_series': 'GDP',
        'result_text': 'GDP thuc te: ${value:,.0f}B (ky truoc: ${prev:,.0f}B)'
    },
    {
        'id': 'fomc_2026-07-30', 'name': '🏦 FOMC Rate Decision (T7)',
        'date': '2026-07-30', 'time': '01:00', 'impact': '🔴 HIGH',
            ...
            if key not in log['events_sent']:
                if check_fred():
                    ...
                    if values and len(values) >= 2:
                        ...
                        result = ev['result_text'].format(value=curr_val, prev=prev_val, action=action)
                        log['events_sent'][key] = time.time()
    ...
    try:
        ...
        for a in articles[:5]:
            ...
            news_list.append({
                ...
                'score': tone, 'gold': gold_impact, 'crypto': crypto_impact,
...
check_fred()
check_gdelt()
print(f"⏱️ Chu ky: {CHU_KY_TIN_TUC}s")
print("=" * 60)
...
if not os.path.exists(STARTUP_FILE):
    ...
    msg += f"📡 FRED: {'✅ Online' if check_fred() else '⏳ Offline'}\n"
    msg += f"📡 GDELT: {'✅ Online' if check_gdelt() else '⏳ Offline'}\n"
    msg += f"\n✅ Dang theo doi..."
    gui_telegram(msg)
